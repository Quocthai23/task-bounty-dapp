...
export const TaskHistory: React.FC = () => {
  ...
  const handleRefresh = async () => {
    await refetch();
    toast.success('Đã cập nhật lịch sử hoạt động mới nhất!');
  };

  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'DONE':
        return <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-100 text-emerald-700 dark:bg-emerald-950/60 dark:text-emerald-300">✅ Hoàn thành</span>;
      case 'REVIEW':
        return <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-100 text-amber-700 dark:bg-amber-950/60 dark:text-amber-300">🔍 Đang Review</span>;
      case 'IN_PROGRESS':
        return <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-blue-100 text-blue-700 dark:bg-blue-950/60 dark:text-blue-300">🚀 Đang thực hiện</span>;
      case 'TODO':
      default:
        return <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300">📝 Cần làm (To Do)</span>;
    }
  };
  ...
  const renderActionBadge = (action: string) => {
    switch (action) {
      ...
      case 'TASK_MOVED':
        return (
          <div className="w-9 h-9 rounded-2xl bg-blue-500/10 text-blue-600 dark:text-blue-400 flex items-center justify-center shrink-0 border border-blue-500/20">
      ...
      case 'TASK_COMMENT_ADDED':
        return (
          <div className="w-9 h-9 rounded-2xl bg-purple-500/10 text-purple-600 dark:text-purple-400 flex items-center justify-center shrink-0 border border-purple-500/20">
      ...
      default:
        return (
          <div className="w-9 h-9 rounded-2xl bg-slate-500/10 text-slate-600 dark:text-slate-400 flex items-center justify-center shrink-0 border border-slate-500/20">
      ...
      <div className="p-5 sm:p-6 lg:p-8 pb-4 bg-white dark:bg-slate-900 border-b border-neutral-200/70 dark:border-slate-800 shrink-0">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400">
              Theo dõi chi tiết tất cả lịch sử kéo task, tạo task, phân công và thảo luận của mọi người trong từng Job.
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-5">
          <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-800/40 border border-slate-200/60 dark:border-slate-800 flex items-center gap-3">
            ...
            <div>
              <span className="text-[10px] uppercase font-bold text-slate-400 block tracking-wider">Tổng Hoạt Động</span>
              <span className="text-xs font-black text-slate-800 dark:text-slate-200">Gần đây</span>
            </div>
          </div>
          ...
          <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-800/40 border border-slate-200/60 dark:border-slate-800 flex items-center gap-3">
            ...
            <div>
              <span className="text-[10px] uppercase font-bold text-slate-400 block tracking-wider">Kéo / Chuyển Cột</span>
              <span className="text-xs font-black text-slate-800 dark:text-slate-200">Trạng thái</span>
            </div>
          </div>
          ...
          <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-800/40 border border-slate-200/60 dark:border-slate-800 flex items-center gap-3">
            ...
            <div>
              <span className="text-[10px] uppercase font-bold text-slate-400 block tracking-wider">Tạo Mới</span>
              <span className="text-xs font-black text-slate-800 dark:text-slate-200">Nhiệm vụ</span>
            </div>
          </div>
          ...
          <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-800/40 border border-slate-200/60 dark:border-slate-800 flex items-center gap-3">
            ...
            <div>
              <span className="text-[10px] uppercase font-bold text-slate-400 block tracking-wider">Bình Luận</span>
              <span className="text-xs font-black text-slate-800 dark:text-slate-200">Thảo luận</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mt-5 pt-4 border-t border-slate-100 dark:border-slate-800">
          ...
          <div className="flex flex-wrap items-center gap-2.5 flex-1 min-w-[280px]">
            ...
            <div className="relative">
              ...
              >
                <option value="ALL">📁 Tất cả Job / Dự án ({allProjects.length})</option>
            ...
            <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800/80 p-1 rounded-xl">
              {[
                { id: 'ALL', label: 'Tất cả' },
                { id: 'TASK_STATUS_CHANGED', label: '🎯 Kéo Task' },
                { id: 'TASK_CREATED', label: '✨ Tạo Task' },
                { id: 'TASK_COMMENT_ADDED', label: '💬 Bình Luận' },
                { id: 'TASK_ASSIGNED', label: '👤 Phân Công' },
              ].map((tab) => (
                <button
                  key={tab.id}
          ...
          <div className="relative w-full sm:w-64">
            <input
              ...
              placeholder="Tìm theo task, người dùng..."
          ...
          {isLoading ? (
            ...
            <div className="text-center py-16 bg-white dark:bg-slate-900 rounded-3xl border border-dashed border-slate-200 dark:border-slate-800 p-8">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 max-w-sm mx-auto mb-6">
                Khi các thành viên tạo task, kéo trạng thái, hoặc thêm bình luận trong Job, toàn bộ lịch sử sẽ xuất hiện tại đây theo thời gian thực.
              </p>
            </div>
          ) : (
            <div className="relative pl-6 sm:pl-8 before:absolute before:left-3 sm:before:left-4 before:top-4 before:bottom-4 before:w-0.5 before:bg-slate-200 dark:before:bg-slate-800 space-y-4">
              ...
              {activities.map((act: any) => {
                ...
                const fullName = user?.firstName ? `${user.firstName} ${user.lastName || ''}`.trim() : (user?.username || user?.email || 'Thành viên');
                const roleTitle = user?.profile?.title || 'Thành viên Job';

                let timeFormatted = 'Vừa xong';
                try {
                  timeFormatted = formatDistanceToNow(new Date(act.createdAt), { addSuffix: true, locale: vi });
                } catch {}
                          ...
                          <div className="text-xs text-slate-700 dark:text-slate-300 leading-relaxed mt-1">
                            ...
                            {act.action === 'TASK_CREATED' && (
                              <div className="space-y-1.5">
                                <p>
                                  đã tạo mới một nhiệm vụ trong Job:{' '}
                                ...
                                {details.priority && (
                                  <div className="flex items-center gap-2 text-[11px] text-slate-500">
                                    <span>Độ ưu tiên: <strong>{details.priority}</strong></span>
                                    {details.assigneeName && <span>• Giao cho: <strong>{details.assigneeName}</strong></span>}
                                  </div>
                                )}
                              </div>
                            )}
                            ...
                            {act.action === 'TASK_COMMENT_ADDED' && (
                              <div className="space-y-2">
                                <p>
                                  đã bình luận trao đổi trong nhiệm vụ{' '}
                                ...
                                <div className="bg-purple-50/60 dark:bg-purple-950/20 border border-purple-100 dark:border-purple-900/40 rounded-2xl p-3 text-slate-800 dark:text-slate-200 font-medium italic text-xs">
                                  "{details.content || details.contentPreview || 'Nội dung bình luận'}"
                                </div>
                              </div>
                            )}
                            ...
                            {act.action === 'TASK_ASSIGNED' && (
                              <p>
                                ...
                                cho <strong>{details.assigneeName || 'Thành viên mới'}</strong>.
                              </p>
                            )}
                            ...
                            {act.action === 'TASK_UPDATED' && (
                              <p>
                                đã cập nhật thông tin nhiệm vụ{' '}
                                <span className="font-bold text-slate-900 dark:text-white">
                                  "{details.taskTitle || 'Nhiệm vụ'}"
                                </span>.
                              </p>
